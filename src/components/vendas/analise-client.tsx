  ...
  return (
    <div className="space-y-4">
      ...
      <Section acc="efic" icon={Zap} title="Anúncios × Plataforma × Vendas" desc="Por plataforma (maior conversão primeiro): anúncios únicos, vendas, conversão (vd/ad) e % das vendas.">
        <div className="overflow-x-auto scroll-thin">
          <table className="w-full text-[13px] min-w-[580px] border-collapse">
            <thead><tr>
              <th className={cn(TH, "text-left")}>Plataforma</th><th className={cn(TH, "text-right")}>Anúncios</th><th className={cn(TH, "text-right")}>Vendas</th><th className={cn(TH, "text-right")}>Comissão</th><th className={cn(TH, "text-right")}>Conversão vd/ad</th><th className={cn(TH, "text-right")}>% vendas</th>
            </tr></thead>
            <tbody>{data.eficiencia.map(e => (
              <tr key={e.source} className="hover:bg-zinc-800/30">
                ...
                <td className={cn(TD, "text-right font-bold text-white")}>{e.vdPorAd == null ? "—" : e.vdPorAd.toFixed(1)}</td>
      ...
      {data.topAnunciados.length > 0 && (
        <Section acc="top" icon={Megaphone} title="Top mais anunciados" desc="O que mais postamos — e se converte (vd/ad, vermelho quando <1).">
          <div className="overflow-x-auto scroll-thin">
