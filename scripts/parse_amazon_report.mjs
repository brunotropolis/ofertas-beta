...
//
// Uso: node scripts/parse_amazon_report.mjs <arquivo.csv> <period_start YYYY-MM-DD> <period_end YYYY-MM-DD>
// Lê a service_role do .env.local; faz upsert direto no Supabase.

import fs from "node:fs";
import path from "node:path";

const [, , file, pStart, pEnd] = process.argv;
if (!file || !pStart || !pEnd) {
  console.error("uso: node scripts/parse_amazon_report.mjs <arquivo.csv> <period_start> <period_end>");
  process.exit(1);
}
...
const out = rows.map((r) => {
  ...
  return {
    external_id: perDay ? `${asin}:${day}` : `${asin}:${pStart}:${pEnd}`,
...
console.log(`OK: ${ok} linhas amazon (perDay=${perDay}, período ${pStart}..${pEnd}). Comissão total: R$ ${out.reduce((a, b) => a + b.commission, 0).toFixed(2)}`);
